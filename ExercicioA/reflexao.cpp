...
int main() {
    int N;
    std::cout << "\033[1;34m--- Reflexão de Sinais Unidimensionais ---\033[0m\n";
    std::cout << "Informe o número de amostras (N): ";
    if (!(std::cin >> N) || N <= 0) {
        std::cerr << "\033[1;31mErro: O tamanho deve ser um número inteiro positivo.\033[0m\n";
        return 1;
    }

    std::vector<Sample> original(N);
    std::vector<Sample> reflected(N);

    std::cout << "Insira as amplitudes para cada índice:\n";
    for (int i = 0; i < N; ++i) {
        original[i].n = i;
        std::cout << "  x[" << i << "] = ";
        if (!(std::cin >> original[i].val)) {
             std::cerr << "Erro na leitura.\n";
             return 1;
        }
    }

    for (int i = 0; i < N; ++i) {
        reflected[i].n = -original[i].n;
        reflected[i].val = original[i].val;
    }
    
    std::sort(reflected.begin(), reflected.end(), [](const Sample& a, const Sample& b) {
        return a.n < b.n;
    });

    // Plotagens ASCII melhoradas
    plotASCII(original, "Sinal Original x[n]", "\033[36m");    // Ciano
    plotASCII(reflected, "Sinal Refletido y[n] = x[-n]", "\033[35m"); // Magenta

    generateSVG(original, "original.svg", "Sinal Original x[n]", "#00d4ff");
    generateSVG(reflected, "refletido.svg", "Sinal Refletido y[n] = x[-n]", "#ff007f");
