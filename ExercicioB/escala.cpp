...
// Função para exibir um gráfico de hastes (stem plot) estilizado no terminal
...
int main() {
    ...
    std::cout << "\033[1;35m--- Mudança de Escala no Tempo ---\033[0m\n";
    std::cout << "Informe o número de amostras original (N): ";
    ...
    std::cout << "Informe o fator de escala (a): ";
    if (!(std::cin >> factor) || factor <= 0) return 1;

    std::vector<Sample> scaled;
    int maxNewN = (int)((N - 1) * factor);
    
    for (int n = 0; n <= maxNewN; ++n) {
        double oldIdx = n / factor;
        double intPart;
        bool isInteger = (std::modf(oldIdx, &intPart) < 0.0001 || std::modf(oldIdx, &intPart) > 0.9999);
        int k = (int)std::round(oldIdx);
        if (isInteger && k >= 0 && k < N) scaled.push_back({n, original[k].val});
        else scaled.push_back({n, 0.0});
    }

    // Plotagens ASCII melhoradas
    plotASCII(original, "Sinal Original x[n]", "\033[36m"); // Ciano
    plotASCII(scaled, "Sinal Escalonado (fator " + std::to_string(factor) + ")", "\033[33m"); // Amarelo

    generateSVG(original, "original.svg", "Sinal Original x[n]", "#00d4ff");
    generateSVG(scaled, "escalonado.svg", "Sinal Escalonado (fator=" + std::to_string(factor) + ")", "#ffaa00");
