...
int main() {
    ...
    std::cout << "Informe o fator de escala de amplitude (A): ";
    if (!(std::cin >> factor)) return 1;

    for (int i = 0; i < N; ++i) {
        modified[i].n = original[i].n;
        modified[i].val = factor * original[i].val;
    }

    // Plotagens ASCII melhoradas
    plotASCII(original, "Sinal Original x[n]", "\033[36m"); // Ciano
    plotASCII(modified, "Sinal Modificado (Amplitude x" + std::to_string(factor) + ")", "\033[32m"); // Verde

    generateSVG(original, "original.svg", "Sinal Original x[n]", "#00d4ff");
    generateSVG(modified, "amplitude_alterada.svg", "Sinal com Amplitude Alterada (A=" + std::to_string(factor) + ")", "#00ff88");
